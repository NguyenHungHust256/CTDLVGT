...
int main(){
	...
	printf("\n>>>>>>Xac dinh duong kinh<<<<<<<<\n");
	printf("\tDuong kinh cua cay da cho la: %3d",duongkinh(randomTree));
//	// Tao cay thu 2
//	printf("\n>>>>>>>> Tree 2<<<<<<<<<<\n");
...
int duongkinh(treeNode *tree){
	...
	int ld=duongkinh(tree->left);
	int rd=duongkinh(tree->right);
	return ((1+hl+hr)>(ld>rd?ld:rd)?(1+hl+hr):(ld>rd?ld:rd));
//	return (1+hl+hr); 
}
